...
// errSummary 把迁移错误压成单行摘要，写入 failedTable.log。
// lib/pq 的 Error.Error() 只返回 Message，这里额外带上 SQLSTATE 错误码
// (如 23502 非空约束冲突、22007 日期格式非法)，便于按错误类型归类排查。
// 其余类型的错误退化为原文并去掉换行，保证日志一行一条。
	...
	for rows.Next() {                                  // 从游标里获取一行行数据
		...
		for i, colValue := range values { //values是完整的一行所有列值，这里从values遍历，获取每一列的值并赋值到col变量，col是单列的列值
			...
			if colValue == nil {
				...
				} else if isZeroDatetime(colType[i], string(colValue)) {
					// MySQL 允许零值日期，PostgreSQL 不允许。原样 COPY 会报
					// invalid input syntax for type timestamp 并中断整表迁移，
					// 这里与建表时丢弃零值默认值的做法保持一致，统一置为 NULL。
					value = nil
				} else {
